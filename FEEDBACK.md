...
- **結果**: **提出物からは取り除いた**（09-26）。配線は動く（本番で 402 を返すところまで
  確認済み）が、この作品には入れていない。理由2つ——①**受取先をリクエストごとに変えられない**
  （`paymentMiddleware` は経路ごとに静的に組む設計）ので、「事務所に渡していない使い方の
  対価は本人に直接」という主張どおりの送金ができない ②本人に直接払うには本人に鍵を持たせる
  必要があり、それはこの設計が意図的に避けた判断と衝突する
- **詰まった所**: ①未入金時のエラー本文がヘッダの base64 の中で、本文は `{}` のみ
  ②**返金・エスクローが無い**ので「聞く前に払う」にできない＝支払いは同意の後にしか
  置けない（設計としては妥当だが、選択の余地が無いのは仕組み側の制約）
- **一番効く改善1つ**: **受取先を動的に決められるようにすること**。「誰に払うか」が
  リクエストごとに変わる用途は想定の外側で、いまはそこで詰まる。サーバ側の楽さ
  （middleware 1本）は本当に良いので、ここが空いているのが惜しい
