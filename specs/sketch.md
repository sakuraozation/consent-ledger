...
1. **Who**: A talent agency that manages models, and the brands / agents that generate images from their body scans. **Revised at 01:00 Sat**: the agency is the operator, not the adversary — a model has an agency because managing their own commercial use is not their job. The adversary is whoever reuses the scan without asking, and today neither of them can act.
2. **The problem** (from a model I know): the body scan itself was taken legitimately, as part of a paid job. What they can't control is what happens **after** — the data gets reused for AI generation without permission. They can't even tell when it happens, because the output doesn't always look like them.
3. **What they actually wants**: not a ban. A **scope and an expiry**, and the ability to take it back.
...
6. **The moment to show**: allow → the agency revokes on their word → the *same* request is now refused. Ten seconds, no explanation needed. The rarer moment, theirs alone: they withdraw the delegation and every consent under it stops at once.
